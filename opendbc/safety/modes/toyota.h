...
#define TOYOTA_BASE_TX_MSGS \
  {0x191, 0, 8, .check_relay = true}, {0x412, 0, 8, .check_relay = true}, {0x1D2, 0, 8, .check_relay = false}, {0x750, 0, 8, .check_relay = false}, /* white list 0x750 for Enhanced Diagnostic Request */  \
...
static bool toyota_tx_hook(const CANPacket_t *msg) {
  ...
  if (msg->addr == 0x750U) {
    ...
    // Enhanced BSM
    bool top_valid_uds_msgs = ((GET_BYTES(msg, 0, 4) == 0x10002141U) ||  // disable left BSM debug
    ...
    top_valid_uds_msgs |= (GET_BYTES(msg, 0, 4) == 0x11300540U) &&       // automatic door locking and unlocking
...
static safety_config toyota_init(uint16_t param) {
  ...
  if (toyota_secoc) {
    ...
    static RxCheck toyota_lka_unsupported_dsu_car_rx_checks[] = {
      TOYOTA_RX_CHECKS(false)
      TOYOTA_DSU_CRUISE_ADDR_CHECK
    };
    static RxCheck toyota_lka_alt_brake_unsupported_dsu_car_rx_checks[] = {
      TOYOTA_ALT_BRAKE_RX_CHECKS(false)
      TOYOTA_DSU_CRUISE_ADDR_CHECK
    };

    if (!toyota_alt_brake) {
      if (toyota_unsupported_dsu_car) {
        SET_RX_CHECKS(toyota_lka_unsupported_dsu_car_rx_checks, ret);
      } else {
        SET_RX_CHECKS(toyota_lka_rx_checks, ret);
      }
    } else {
      if (toyota_unsupported_dsu_car) {
        SET_RX_CHECKS(toyota_lka_alt_brake_unsupported_dsu_car_rx_checks, ret);
      } else {
...
static bool toyota_fwd_hook(int bus_num, int addr) {
  ...
  if (bus_num == 2) {
    ...
    bool is_aeb_msg = ((msg->addr == 0x344U) && (alternative_experience & ALT_EXP_ALLOW_AEB));
